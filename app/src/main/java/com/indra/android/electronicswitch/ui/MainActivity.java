...
public class MainActivity extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private void getAllFeeds() {
        getAllFeedsFromServer().setRetryPolicy(new DefaultRetryPolicy(DefaultRetryPolicy.DEFAULT_TIMEOUT_MS,
    ...
    private void addFeed1ToServer() {
        ...
        stringRequest4.setRetryPolicy(new DefaultRetryPolicy(DefaultRetryPolicy.DEFAULT_TIMEOUT_MS,
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
    ...
    private void addData1ToServer() {
        ...
        loadingDialog2.show();
        StringRequest stringRequest4 = new StringRequest(Request.Method.POST, Constant.ADD_DATA1_URL +
                usernameaio + "/feeds/switch1/data",
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            ...
                            if (!obj.getBoolean("error")) {
                                ...
                                Toast.makeText(getApplicationContext(), obj.getString("message"),
                                        Toast.LENGTH_SHORT).show();
                            } } catch (JSONException e) { e.printStackTrace(); }loadingDialog2.dismiss(); }},
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
